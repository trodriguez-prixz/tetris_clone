...
export default class OverlayRenderer {
  ...
  renderStartScreen() {
    ...
    const titleText = this.createCenteredText(OVERLAY_TEXT_LAYOUT.startTitle, 'TETRIS', COLORS.DANGER, 'bold');
    const startText = this.createCenteredText(OVERLAY_TEXT_LAYOUT.startPrompt, 'Presiona cualquier tecla', COLORS.SUCCESS, 'bold');
    this.scene.tweens.add({ targets: startText, alpha: START_PROMPT_FLASH_ALPHA, duration: START_PROMPT_FLASH_DURATION, yoyo: true, repeat: REPEAT_FOREVER });
  ...
  renderPauseScreen() {
    ...
    const text = this.createCenteredText(OVERLAY_TEXT_LAYOUT.pauseTitle, 'PAUSED', COLORS.WARNING, 'bold');
  ...
  renderGameOverScreen() {
    ...
    const overlay = this.createOverlay(MODAL_OVERLAY_ALPHA);
    const text = this.createCenteredText(OVERLAY_TEXT_LAYOUT.gameOverTitle, 'GAME OVER', COLORS.DANGER, 'bold');
    const restartText = this.createCenteredText(OVERLAY_TEXT_LAYOUT.restartPrompt, 'Press R to Restart', COLORS.SECONDARY_TEXT);
  ...
  createOverlay(alpha) {
    return this.scene.add.rectangle(CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2, CANVAS_WIDTH, CANVAS_HEIGHT, COLORS.OVERLAY, alpha);
  }
  ...
  createCenteredText(layout, text, fill, fontStyle) {
    ...
    return this.scene.add.text(CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 + layout.offsetY, text, style).setOrigin(CENTER_ORIGIN);
  }
