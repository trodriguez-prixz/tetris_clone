...
export class RetroMusic {
  ...
  calculateSongDurations() {
    return this.songs.map((song) => {
      const melodyDuration = song.melody.reduce(
        (sum, note) => sum + note[1],
        0
      );
      const bassDuration = song.bassLine.reduce(
        (sum, note) => sum + note[1],
        0
      );
  ...
  scheduleNextSong() {
    if (!this.isPlaying || this.isPaused || !this.scene || !this.scene.time)
      return;
  ...
  playMelody() {
    ...
    const note =
      this.currentSong.melody[
        this.currentNoteIndex % this.currentSong.melody.length
      ];
  ...
  playBass() {
    ...
    const bassIndex =
      Math.floor(this.currentNoteIndex / 2) % this.currentSong.bassLine.length;
